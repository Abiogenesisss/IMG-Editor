...
def run_parallel(fn, args_list, progress_cb=None):
    """通用并行调度：fn(*args) 逐项提交到进程池，返回结果列表（保持原顺序）
    progress_cb(done, total) 每完成一项时回调
    进程池崩溃时自动重建并重试一次
    支持通过 _cancel_event 优雅取消：不再提交新任务，已提交的等待完成"""
    total = len(args_list)
    results = [None] * total
    _cancel_event.clear()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if handler:
            try:
                ...
                try:
                    self._json({"success": False, "error": tb}, 500)
                except Exception:
                    pass
        else:
    ...
    def _deduplicate(self):
        ...
        duplicates = find_duplicates_perceptual(feat_results,
    ...
    def _cluster(self):
        ...
        try:
            ...
            self._send_event("done", {"success": False, "error": traceback.format_exc()})
