...
function loadMenuConfig() {
  try {
    ...
    const result = {}
    allMenuItems.forEach((item) => { result[item.path] = true })
    return result
  }
}
...
const updateStatusText = computed(() => {
  switch (updateStatus.value) {
    ...
    case 'downloading': return `正在下载更新... ${updateInfo.value?.percent || 0}%`
    case 'downloaded': return '下载完成，点击安装并重启'
    case 'error': return `检查更新失败: ${updateInfo.value?.message || '未知错误'}`
    default: return ''
  }
})
...
function notifyApiConfigsChanged() {
  window.dispatchEvent(new CustomEvent('api-configs-updated'))
}

function serializeConfigs(list) {
...
async function saveConfigs(list) {
  apiConfigs.value = await window.api.saveApiConfigs(serializeConfigs(list))
  notifyApiConfigsChanged()
}

async function refreshConfigs() {
  apiConfigs.value = await loadConfigs()
}
...
<template>
  <div class="settings-page">
    <div class="settings-content">
        ...
        <div class="menu-grid">
          ...
          >
            ...
            <div class="menu-card-icon">
              <!-- 图片抓取 -->
              <svg v-if="item.icon === 'grab'" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>
              <!-- 图片处理 -->
              <svg v-else-if="item.icon === 'process'" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 2 7 12 12 22 7 12 2"></polygon><polyline points="2 17 12 22 22 17"></polyline><polyline points="2 12 12 17 22 12"></polyline></svg>
              <!-- 数据增强 -->
              <svg v-else-if="item.icon === 'augment'" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"></polyline><polyline points="17 6 23 6 23 12"></polyline></svg>
              <!-- 超分辨率 -->
              <svg v-else-if="item.icon === 'upscale'" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 3 21 3 21 9"></polyline><polyline points="9 21 3 21 3 15"></polyline><line x1="21" y1="3" x2="14" y2="10"></line><line x1="3" y1="21" x2="10" y2="14"></line></svg>
              <!-- 图片打标 -->
      ...
      <section class="settings-section">
        <div class="section-header">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="6" width="20" height="12" rx="2"></rect><path d="M6 12h.01"></path><path d="M10 12h.01"></path><path d="M14 12h.01"></path><path d="M18 12h.01"></path><line x1="6" y1="2" x2="6" y2="6"></line><line x1="18" y1="2" x2="18" y2="6"></line></svg>
          <h2>GPU 加速</h2>
        </div>
        ...
        <div v-if="gpuInfo" class="gpu-status-card" :class="{ available: gpuInfo.cuda_available }">
          <div class="gpu-status-icon">
            <svg v-if="gpuInfo.cuda_available" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>
            <svg v-else width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg>
          </div>
          <div class="gpu-status-info">
            ...
            <span v-if="gpuInfo.cuda_available && gpuInfo.vram_total" class="gpu-vram">
              显存: {{ gpuInfo.vram_free != null ? gpuInfo.vram_free + ' / ' : '' }}{{ gpuInfo.vram_total }} GB
            </span>
            <span v-if="!gpuInfo.cuda_available" class="gpu-vram">未检测到支持 CUDA 的 GPU，所有任务将使用 CPU</span>
          </div>
        </div>
        ...
        <!-- GPU 开关 -->
        <div class="setting-row" v-if="gpuInfo && gpuInfo.cuda_available">
          <div class="setting-info">
            <span class="setting-label">启用 GPU 加速</span>
            <span class="setting-desc">开启后超分辨率、打标、裁剪、聚类等任务将使用 GPU 加速处理</span>
          </div>
        ...
        <div v-if="updateStatus" class="update-feedback">
          <div :class="['update-status-bar', updateStatus]">
            ...
            <div class="update-actions">
              <button v-if="updateStatus === 'available'" class="update-action-btn" @click="downloadUpdate">下载更新</button>
              <button v-if="updateStatus === 'downloaded'" class="update-action-btn" @click="installUpdate">安装并重启</button>
              <button
          ...
          <div v-if="updateStatus === 'downloading' && updateInfo?.percent != null" class="update-progress-track">
      ...
      <section class="settings-section">
        <div class="section-header">
          ...
          <h2>API 配置 <span class="config-count">({{ apiConfigs.length }})</span></h2>
          ...
          <div v-if="showForm" class="config-form">
            ...
            <div class="form-row two-col">
              <div class="form-field">
                ...
                <input class="form-input" type="text" v-model="formName" placeholder="Deepseek" />
              </div>
              <div class="form-field">
                <label class="form-label">MODEL NAME</label>
                <input class="form-input" type="text" v-model="formModel" placeholder="deepseek-reasoner" />
              </div>
            </div>
            <div class="form-field">
              ...
              <div class="form-input-icon">
                ...
                <input class="form-input with-icon" type="text" v-model="formEndpoint" placeholder="https://api.deepseek.com/v1" />
              </div>
            ...
            <div class="form-field">
              ...
              <div class="form-input-icon right">
                <input class="form-input with-icon-right" :type="showApiKey ? 'text' : 'password'" v-model="formApiKey" placeholder="" />
                <button class="eye-btn" @click="showApiKey = !showApiKey">
                  <svg v-if="!showApiKey" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
        ...
        <TransitionGroup name="list-fade" tag="div" class="config-list">
          <div v-for="cfg in apiConfigs" :key="cfg.id" :class="['config-item', { disabled: !cfg.enabled }]">
...
.add-config-btn:hover { opacity: 0.85; }
...
.check-btn:hover:not(:disabled) { border-color: var(--color-text); background: var(--color-surface-soft); }
...
.update-status-bar.checking { color: var(--color-text-secondary); }
.update-status-bar.available { color: var(--color-info); background: var(--color-info-light); border-color: var(--color-border); }
.update-status-bar.not-available { color: var(--color-success); background: var(--color-tag-has-bg); border-color: var(--color-tag-has-border); }
.update-status-bar.downloading { color: var(--color-text-secondary); }
.update-status-bar.downloaded { color: var(--color-info); background: var(--color-info-light); border-color: var(--color-border); }
.update-status-bar.error { color: var(--color-error); background: var(--color-error-light); border-color: var(--color-error-border); }
.update-status-text {
  font-size: 12px;
  font-weight: 500;
}
...
.update-action-btn:hover { opacity: 0.85; }
.update-dismiss-btn {
  padding: 4px 10px;
...
.update-dismiss-btn:hover { border-color: var(--color-text-muted); }
...
.form-input-icon.right > svg { display: none; }
...
.config-item:hover { border-color: var(--color-border-unchecked); }
...
.config-toggle.on { background: #7C3AED; }
.config-toggle-knob {
  position: absolute;
  top: 2px;
  left: 2px;
...
.config-toggle.on .config-toggle-knob { transform: translateX(16px); }
...
.config-action-btn:hover { color: var(--color-text-secondary); background: var(--color-surface-hover); }
.config-action-btn.delete:hover { color: var(--color-error); background: var(--color-error-light); }
...
[data-theme="dark"] .menu-card.active {
  ...
  border-color: #818cf8;
  box-shadow: 0 0 0 1px rgba(129, 140, 248, 0.3), 0 0 12px rgba(99, 102, 241, 0.08);
}
