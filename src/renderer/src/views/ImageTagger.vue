...
async function runTag() {
  ...
  try {
    ...
    if (processingAction.value === 'tag') processingAction.value = null
...
async function saveAllTags() {
  ...
  if (Object.keys(toSave).length === 0) {
    for (const [path, tags] of Object.entries(tagsMap.value)) {
      if (tags) toSave[path] = tags
    }
  }
...
const sortedTagList = computed(() => {
  ...
  if (tagSortBy.value === 'name') {
    entries.sort((a, b) =>
      tagSortOrder.value === 'asc' ? a[0].localeCompare(b[0]) : b[0].localeCompare(a[0])
    )
  } else {
...
function runBatchEdit() {
  ...
  if (batchMode.value === 'prepend' || batchMode.value === 'append') {
    ...
    for (const path of targets) {
      ...
      newMap[path] = joinTags(
        batchMode.value === 'prepend' ? [...toInsert, ...existing] : [...existing, ...toInsert]
      )
      newDirty.add(path)
    }
  } else if (batchMode.value === 'replace') {
    const from = batchReplaceFrom.value.trim(),
      to = batchReplaceTo.value.trim()
    if (!from) return
    for (const path of targets) {
      const existing = splitTags(newMap[path] || '')
      let changed = false
...
function removeTagFromAll(tagName) {
  ...
  if (editingImage.value) editingTags.value = newMap[editingImage.value.path] || ''
  // 同时从筛选中移除
  const fs = new Set(filterTags.value)
  fs.delete(tagName)
  filterTags.value = fs
}
  ...
  <div class="process-page tagger-page">
      ...
      <div class="action-section">
        <div class="model-select-wrap">
          ...
          <button
            ...
            :disabled="downloading || !!processingAction"
            @click="downloadModel"
          >
        ...
        <button
          class="action-btn"
          :disabled="imageCount === 0 || !!processingAction || !currentModelInfo?.downloaded"
          @click="runTag"
        >
          开始打标
        </button>
    ...
    <div class="threshold-bar">
      ...
      <label class="threshold-toggle" @click="replaceUnderscore = !replaceUnderscore">
        <span :class="['toggle-switch', { on: replaceUnderscore }]"
          ><span class="toggle-knob"></span
        ></span>
        <span class="toggle-text">下划线转空格</span>
      </label>
      <label
        :class="['threshold-toggle', 'cl-only', { dim: !isClTagger }]"
        @click="keepCopyright = !keepCopyright"
      >
        <span :class="['toggle-switch', { on: keepCopyright && isClTagger }]"
          ><span class="toggle-knob"></span
        ></span>
      ...
      <label
        :class="['threshold-toggle', 'cl-only', { dim: !isClTagger }]"
        @click="keepRating = !keepRating"
      >
        <span :class="['toggle-switch', { on: keepRating && isClTagger }]"
          ><span class="toggle-knob"></span
        ></span>
      ...
      <label
        :class="['threshold-toggle', 'cl-only', { dim: !isClTagger }]"
        @click="keepMeta = !keepMeta"
      >
        <span :class="['toggle-switch', { on: keepMeta && isClTagger }]"
          ><span class="toggle-knob"></span
        ></span>
      ...
      <label
        :class="['threshold-toggle', 'cl-only', { dim: !isClTagger }]"
        @click="keepModel = !keepModel"
      >
        <span :class="['toggle-switch', { on: keepModel && isClTagger }]"
          ><span class="toggle-knob"></span
        ></span>
      ...
      <label
        :class="['threshold-toggle', 'cl-only', { dim: !isClTagger }]"
        @click="keepQuality = !keepQuality"
      >
        <span :class="['toggle-switch', { on: keepQuality && isClTagger }]"
          ><span class="toggle-knob"></span
        ></span>
    ...
    >
      ...
      <template #idle>
        共 {{ imageCount }} 张<template v-if="filterTags.size > 0"
          >，筛选 {{ filteredImages.length }} 张</template
        >，已选 {{ selectedCount }} 张
      </template>
    </ImageStatusBar>
    ...
    <div v-if="imageCount > 0" class="tagger-body">
      ...
      <div v-if="showTagPanel" class="tag-panel">
        ...
        <!-- 搜索 -->
        <input v-model="tagSearch" class="tp-search" placeholder="搜索标签..." />
        ...
        <div class="tp-controls">
          <div class="tp-control-row">
            ...
            <div class="tp-sort-group">
              <label
                :class="['tp-chip', { active: tagSortBy === 'frequency' }]"
                @click="tagSortBy = 'frequency'"
                >频次</label
              >
              <label
                :class="['tp-chip', { active: tagSortBy === 'name' }]"
                @click="tagSortBy = 'name'"
                >名称</label
              >
            </div>
            <div class="tp-sort-group">
              <label
                :class="['tp-chip', { active: tagSortOrder === 'desc' }]"
                @click="tagSortOrder = 'desc'"
                >降序</label
              >
              <label
                :class="['tp-chip', { active: tagSortOrder === 'asc' }]"
                @click="tagSortOrder = 'asc'"
                >升序</label
              >
            </div>
          </div>
          <div class="tp-control-row">
            ...
            <div class="tp-sort-group">
              <label
                :class="['tp-chip', { active: filterLogic === 'and' }]"
                @click="filterLogic = 'and'"
                >AND</label
              >
              <label
                :class="['tp-chip', { active: filterLogic === 'or' }]"
                @click="filterLogic = 'or'"
                >OR</label
              >
              <label
                :class="['tp-chip', { active: filterLogic === 'none' }]"
                @click="filterLogic = 'none'"
                >NONE</label
              >
            </div>
            <button v-if="filterTags.size > 0" class="tp-clear-btn" @click="clearFilterTags">
              清除
            </button>
          </div>
        </div>
          ...
          <div class="tp-tag-chips-wrap">
            <span
              ...
              :class="[
                'tp-tag-chip',
                {
                  filtered: filterTags.has(tagName),
                  'has-tag': editingImage && editingHasTag(tagName)
                }
              ]"
        ...
        <div class="tp-batch">
          <div class="tp-batch-header">
            ...
            <span class="tp-batch-scope">{{
              selectedCount > 0 ? `${selectedCount} 张选中` : `全部 ${imageCount} 张`
            }}</span>
          </div>
          <div class="tp-batch-modes">
            <label
              :class="['tp-chip', { active: batchMode === 'prepend' }]"
              @click="batchMode = 'prepend'"
              >前插</label
            >
            <label
              :class="['tp-chip', { active: batchMode === 'append' }]"
              @click="batchMode = 'append'"
              >后插</label
            >
            <label
              :class="['tp-chip', { active: batchMode === 'replace' }]"
              @click="batchMode = 'replace'"
              >替换</label
            >
          </div>
          <template v-if="batchMode !== 'replace'">
            <div class="tp-batch-input-row">
              <input
                v-model="batchInput"
                class="tp-batch-input"
                placeholder="输入标签, 逗号分隔"
                @keyup.enter="runBatchEdit"
              />
              <button class="tp-batch-run" :disabled="!batchInput.trim()" @click="runBatchEdit">
                执行
              </button>
            </div>
          </template>
          <template v-else>
            <div class="tp-batch-input-row">
              <input v-model="batchReplaceFrom" class="tp-batch-input" placeholder="原标签" />
            </div>
            <div class="tp-batch-input-row">
              <input
                v-model="batchReplaceTo"
                class="tp-batch-input"
              ...
              <button
                class="tp-batch-run"
                :disabled="!batchReplaceFrom.trim()"
                @click="runBatchEdit"
              >
                执行
              </button>
      ...
      <div ref="gridRef" class="image-grid tagger-grid">
        ...
        >
          ...
          <span v-if="getTagPreview(img.path)" class="tag-badge">{{
            getTagPreview(img.path)
          }}</span>
...
.threshold-toggle:hover .toggle-text {
  color: var(--color-text);
}
.threshold-toggle.cl-only.dim {
  opacity: 0.4;
}
.threshold-toggle.cl-only.dim:hover .toggle-text {
  color: var(--color-text-secondary);
}
.toggle-switch {
  position: relative;
...
.tp-tag-chip.filtered {
  background: var(--color-tag-filtered-bg);
  border-color: var(--color-tag-hover-border);
  color: var(--color-tag-filtered-color);
}
.tp-tag-chip.has-tag {
